package greyvarEditor.components;
...
public class ComponentGridArea extends JComponent implements MouseListener, MouseMotionListener {
	...
	public ComponentGridArea(ImageMatrixCellChangeListener l) {
		this.listener = l;
	...
	public void blanketTextureEverything(Texture t) {
		for (int i = 0; i < GridFile.GRID_SIZE; i++) {
			for (int j = 0; j < GridFile.GRID_SIZE; j++) {
	...
	public Point getTileAt(Point p) {
		int w = this.getWidth() / GridFile.GRID_SIZE;
		int h = this.getHeight() / GridFile.GRID_SIZE;
	...
	public void mouseMoved(MouseEvent e) {
		...
		if ((p.x < GridFile.GRID_SIZE) && (p.y < GridFile.GRID_SIZE)) {
